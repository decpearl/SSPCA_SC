function [ U, V, Eta, params, params_V, params_U] = sspca( X, spG, params ) 
...
if (~issparse(spG)) || ( size(spG,1) ~= p ) || any( isnan(spG(:)) ) || any( isinf(spG(:)) ),
    error('*** The group matrix G is not correctly defined ***');
end
...
if isfield(params,'m') && ( mod(r,params.m)==0 ), 
    m           = params.m;
    spG_squared = repmat( spG_squared, [r/m, 1] );
else
    m = r;
end
%--------------------------------------------------------------------------
...
while ( t<=max_it ) && ( delta_cost > min_delta_cost ),
    ...
    [inv_Zeta, Eta] = mexUpdateEta( reshape(V,[r/m*p,m]), spG_squared, params_Eta );
...
% to the objective function *without* the variational reformulation of Omega
    function display_cost_function
        ...
        if ( mod(t,it0) == 0 ),
          ...
          Omega_exact  = mexGetOmega(reshape(V,[r/m*p,m]),spG_squared,params_Eta);
          Omega_approx = get_Omega_from_eta;

...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
   ...
   dcopy ( &pr, V, &one_intt, V_output, &one_intt );
   //-----------------------------------------------------------------------
   ...
   // XtU <- X^T*U
   dgemm ("T","N",&p,&r,&n, &one_db,X,&n,U,&n,&zero_db, XtU,&p);
 
   //UtU <- U^T*U
   dsyrk ( "L", "T", &r, &n, &one_db, U, &n, &zero_db, UtU, &r );
   ...
   for (intt t = 1; t <= max_it; t ++ )
      for (intt col = 0; col < r; col++) 
      {
       ...
       //------------------------------------------------------------------
        ...
        // Tmp <-   XtU(:,k)
        dcopy ( &p, XtU+col*p, &one_intt, tmp, &one_intt );
        
        // Tmp <-   -V * UtU(:,k) + Tmp
        dgemv ("N", &p, &r, &minus_one_db, V_output, &p, UtU+col*r, &one_intt, &one_db, tmp, &one_intt );
       
        // Tmp <- Tmp + UtU_kk*V_output(:,k);
        daxpy (&p, &UtU_kk, V_output+col*p, &one_intt, tmp, &one_intt );  
